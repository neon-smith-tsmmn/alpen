...
use crate::txs::test_utils::TEST_MAGIC_BYTES;

// Helper function to create tagged payload with custom parameters
pub(crate) fn create_tagged_payload(subprotocol_id: u8, tx_type: u8, aux_data: Vec<u8>) -> Vec<u8> {
...
pub(crate) fn mutate_op_return_output(tx: &mut Transaction, tagged_payload: Vec<u8>) {
    tx.output[0].script_pubkey =
        ScriptBuf::new_op_return(PushBytesBuf::try_from(tagged_payload).unwrap());
}

// Helper function to parse transaction
pub(crate) fn parse_tx(tx: &Transaction) -> TxInputRef<'_> {
