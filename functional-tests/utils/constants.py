BD_USERNAME = "alpen"
BD_PASSWORD = "alpen"
DD_ROOT = "_dd"
# keep in sync with `strata-consensus-logic::genesis::MAX_HORIZON_POLL_INTERVAL`
MAX_HORIZON_POLL_INTERVAL_SECS = 1
SEQ_SLACK_TIME_SECS = 2  # to account for thread sync and startup times
BLOCK_GENERATION_INTERVAL_SECS = 0.5
SEQ_PUBLISH_BATCH_INTERVAL_SECS = 5

# Error codes
ERROR_PROOF_ALREADY_CREATED = -32611
ERROR_CHECKPOINT_DOESNOT_EXIST = -32610

# custom precompiles
PRECOMPILE_BRIDGEOUT_ADDRESS = "0x5400000000000000000000000000000000000001"
PRECOMPILE_SCHNORR_ADDRESS = "0x5400000000000000000000000000000000000002"

# Unspendable address
# Taken from python-strata-utils:
UNSPENDABLE_ADDRESS = "bcrt1p7hgsjwtz2pkz45y97dglj4yuc88zsva2p0n5tmcz0zrvfmhcc2lsckedfk"

# SATS to WEI
SATS_TO_WEI = 10_000_000_000
GWEI_TO_WEI = 1_000_000_000

# Network times and stuff
DEFAULT_BLOCK_TIME_SEC = 1
DEFAULT_EPOCH_SLOTS = 64
DEFAULT_HORIZON_HT = 90
DEFAULT_GENESIS_TRIGGER_HT = 100
DEFAULT_OPERATOR_CNT = 2
DEFAULT_PROOF_TIMEOUT = 30  # Secs
DEFAULT_MESSAGE_INTERVAL_MSEC = 500
DEFAULT_TAKEBACK_TIMEOUT = 1_008  # Blocks (1 week)

# Default prover config
DEFAULT_PROVER_NATIVE_WORKERS = 20
DEFAULT_PROVER_POLLING_INTERVAL = 100
DEFAULT_PROVER_ENABLE_CHECKPOINT_PROVING = False
DEFAULT_PROVER_MAX_RETRY_COUNTER = 15
